...
$('detailCard').innerHTML=`<div class="detail-head"><h2 class="detail-title">${s.name}</h2><button class="detail-close" aria-label="閉じる">×</button></div><div class="detail-section detail-team-section"><p class="detail-label">使用チーム</p><div class="detail-teams">${s.clubs.map(t=>{const l=teamLeague[t]||s.leagues[0],done=visitedTeams.includes(t);return `<button class="detail-team detail-team-${l} ${done?'done':''}" data-team="${t}"><span class="detail-team-check">${done?'✓':''}</span><span>${t}</span><small>${l}</small></button>`}).join('')}</div></div><div class="detail-section"><div class="detail-location"><p class="detail-label">所在地</p><strong class="detail-prefecture">${s.area}</strong><span class="detail-address">${address}</span></div></div><div class="detail-section"><p class="detail-label">訪問記録</p><button class="detail-action detail-visit ${visited.includes(s.id)?'done':''}" data-id="${s.id}">${visited.includes(s.id)?'✓ 行ったスタジアム':isAdmin?'行ったスタジアムに登録':'行っていないスタジアム'}</button></div><div class="detail-section"><p class="detail-label">アクセス</p>${access?`<p class="access-text">${access.text}</p>${access.notice?`<p class="access-notice">⚠️ ${access.notice}</p>`:''}`:'<p class="access-text">最寄り駅からのアクセス情報は順次追加します。</p>'}<iframe class="stadium-map" src="${pin}" loading="lazy" title="${s.name}の地図"></iframe><a class="detail-action map-button" href="${route}" target="_blank" rel="noopener">🧭 現在地からの経路をGoogleマップで開く</a>${access?`<a class="detail-action official-button" href="${access.url}" target="_blank" rel="noopener">公式ページでアクセス情報を確認</a>`:''}<p class="detail-note">所要時間は目安です。試合日は混雑や交通規制により変わる場合があります。</p></div>`;
...
function enableDetailSwipe(){const card=$('detailCard');let startY=0,dragY=0,swiping=false;
card.ontouchstart=e=>{if(card.scrollTop>0)return;startY=e.touches[0].clientY;dragY=0;swiping=true;card.classList.add('swiping')};
card.ontouchmove=e=>{if(!swiping)return;dragY=Math.max(0,e.touches[0].clientY-startY);if(dragY>0){card.style.transform=`translateY(${dragY}px)`;e.preventDefault()}};
card.ontouchend=()=>{if(!swiping)return;swiping=false;card.classList.remove('swiping');if(dragY>=90)closeDetail();else card.style.transform=''}}
function closeDetail(){const card=$('detailCard');card.style.transform='';card.classList.remove('swiping');$('detailOverlay').classList.remove('open');document.body.style.overflow=''}
