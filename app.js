...
return{id:String(id),name,clubs:club.split('／'),league,area}}),teams=data.flatMap(s=>s.clubs);
...
$('items').innerHTML=shown.map(s=>`<div class="stadium ${visited.includes(s.id)?'done':''}"><div class="stadium-main"><button class="visit" data-id="${s.id}" aria-label="${s.name}へ行った">${visited.includes(s.id)?'✓':''}</button><span class="info"><span class="name">${s.name}</span><span class="area">${s.area}</span></span><span class="badge ${s.league}">${s.league}</span></div><div class="teams"><span class="teams-label">行ったチーム</span>${s.clubs.map(t=>`<button class="team ${visitedTeams.includes(t)?'done':''}" data-team="${t}">${visitedTeams.includes(t)?'✓ ':''}${t}</button>`).join('')}</div></div>`).join('');
