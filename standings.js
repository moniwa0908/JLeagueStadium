...
function renderStandings(){
  ...
  $('standingsItems').innerHTML=`<table class="standings-table"><thead><tr><th>順位</th><th>チーム</th><th>勝点</th><th>試合</th><th>勝</th><th>分</th><th>敗</th><th>得点</th><th>失点</th><th>得失差</th></tr></thead><tbody>${rows.map(row=>`<tr><td>${standingsEscape(row.rank)}</td><td><button class="standing-team" data-team="${standingsEscape(row.team)}">${standingsEscape(row.team)}</button></td><td class="standing-points">${standingsEscape(row.points)}</td><td>${standingsEscape(row.played)}</td><td>${standingsEscape(row.won)}</td><td>${standingsEscape(row.drawn)}</td><td>${standingsEscape(row.lost)}</td><td>${standingsEscape(row.goalsFor)}</td><td>${standingsEscape(row.goalsAgainst)}</td><td>${standingsEscape(row.goalDifference)}</td></tr>`).join('')}</tbody></table>`;
